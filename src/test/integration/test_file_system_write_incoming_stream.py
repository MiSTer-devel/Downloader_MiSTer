...
class TestWriteIncomingStream(unittest.TestCase):
    ...
    def sut(self, time_monotonic=time.monotonic):
        ...
        config['base_path'] = self.tempdir.name
        return make_production_filesystem_factory(config, time_monotonic=time_monotonic).create_for_system_scope()

    def target(self):
        return self.tempdir.name + '/target.bin'

    def test_write_incoming_stream___with_data___returns_correct_size_and_md5(self):
        data = b'hello world'
    ...
    def test_write_incoming_stream___when_stalled_beyond_timeout___raises_timeout_error(self):
        clock = [0.0]
        def fake_monotonic():
            clock[0] += 100.0
            return clock[0]
        ...
        with self.assertRaises(FsTimeoutError):
            self.sut(time_monotonic=fake_monotonic).write_incoming_stream(stream, self.target(), 0)

    def test_write_incoming_stream___when_stall_recovers_within_timeout___returns_data(self):
        clock = [0.0]
        def fake_monotonic():
            clock[0] += 1.0
            return clock[0]
