...
from test.objects import db_test_descr
from test.fake_db_gateway import DbGateway
from test.fake_file_system import fake_temp_file, FileSystem
from test.fakes import FactoryStub
...
class TestDbGateway(unittest.TestCase):
    ...
    def test_fetch_all___db_with_fs_path___returns_expected_db(self):
        ...
        self.assertEqual(db_test_descr(), fetch_all(fs_db_path, fs))
    ...
    def test_fetch_all___db_with_failing_http_uri___returns_none(self):
        factory = FactoryStub(FileDownloader()).has(lambda fd: fd.test_data.errors_at(fake_temp_file))
        self.assertEqual(None, fetch_all(http_db_url, factory=factory))


def fetch_all(db_url, fs=None, factory=None):
    dbs, errors = DbGateway(file_system=fs, file_downloader_factory=factory).fetch_all(single_db(db_url))
    if len(dbs) == 0:
        return None
    return dbs[0][1]


def single_db(db_uri):
    return {'single': {'section': 'single', 'db_url': db_uri}}
